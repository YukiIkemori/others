#!/usr/bin/env node
// Decor-aware checks for the アルカナ maps (castle, town, house interiors):
...
//   node tools/fixtures/towns/arcana/check.js      (exit 1 on errors)
...
const MAPS = ['arcana_city', 'arcana_house_metem'];
const errors = [], warns = [];
...
console.log(`\narcana check: ${errors.length} error(s), ${warns.length} warning(s)`);
