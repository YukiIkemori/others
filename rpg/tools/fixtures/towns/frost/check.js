#!/usr/bin/env node
// Decor-aware checks for the フロスト maps (village + house interiors):
...
//   node tools/fixtures/towns/frost/check.js      (exit 1 on errors)
...
const MAPS = ['frost_village', 'frost_house_elder', 'frost_house'];
const errors = [], warns = [];
...
console.log(`\nfrost check: ${errors.length} error(s), ${warns.length} warning(s)`);
