...
(function (R) {
  ...
  const act = (name, job, target, effects, o) => Object.assign({ name, job, kind: 'action', jp: 50, desc: name + 'の テスト。', target, effects }, o || {});
  ...
  Object.assign(DB.abilities, {
    ...
    tb_double:  act('にだんぎり', 'tb_fighter', 'enemy', [dmg('phys', 0.8, { hits: 2 })], { mp: 3, fx: 'slash' }),
    tb_drain:   act('きゅうけつぎり', 'tb_fighter', 'enemy', [dmg('phys', 1, { drain: 0.5 })], { mp: 3, fx: 'drain' }),
    tb_sacrifice: act('すてみ', 'tb_fighter', 'enemy', [dmg('phys', 2.5, { hpCost: 0.1, ignoreDef: true })], { fx: 'slash' }),
    tb_holyblade: act('せいなるやいば', 'tb_fighter', 'enemy', [dmg('phys', 1, { element: 'holy', vs: { undead: 2 } })], { mp: 2, fx: 'holy' }),
    tb_steal:   act('ぬすむ', 'tb_fighter', 'enemy', [{ type: 'steal' }], { fx: 'steal' }),
    tb_mug:     act('ごうだつ', 'tb_fighter', 'enemy', [{ type: 'steal', rareBonus: 1 }], { fx: 'steal' }),
    tb_scan:    act('しらべる', 'tb_fighter', 'enemy', [{ type: 'scan' }], { fx: 'scan' }),
    tb_flurry:  act('みだれうち', 'tb_fighter', 'random', [dmg('phys', 0.7, { hits: 4 })], { mp: 4, fx: 'pierce' }),
    tb_cheer:   act('ときのこえ', 'tb_fighter', 'allies', [{ type: 'buff', stat: 'atk', stages: 1 }], { mp: 3, fx: 'buff' }),
    tb_seed:    act('ひみつのたね', 'tb_fighter', 'ally', [{ type: 'grow', stat: 'str', n: 2 }], { fx: 'grow' }),
    tb_smoke:   act('えんまく', 'tb_fighter', 'self', [{ type: 'escape' }], { fx: 'smoke' }),
    tb_osmose:  act('マナドレイン', 'tb_fighter', 'enemy', [dmg('magic', 6, { mp: true, drain: 1, scale: 0.2 })], { mp: 0, magic: true, fx: 'drain' }),
    tb_special: act('ひっさつ', 'tb_fighter', 'enemy', [{ type: 'special', id: 'tb_special' }], { fx: 'strike' }),
    tb_warpout: act('テレポ', 'tb_fighter', 'self', [{ type: 'teleport' }], { fieldUse: true, fx: 'warp' }),
    // reactions / supports
    ...
    tb_rage:    react('いかり', 'hitAny', { type: 'buff', stat: 'atk', stages: 1 }),
    tb_cover:   react('かばう', 'allyLowHp', { type: 'cover' }),
    tb_guardian: react('ガーディアン', 'allyLowHp', { type: 'heal', pct: 0.25 }),
    tb_phoenix: react('ふしちょう', 'ko', { type: 'revive', pct: 0.5 }),
    tb_manashield: react('マナリカバー', 'hitMagic', { type: 'mp', power: 7 }),
    tb_twoswords: { name: 'にとうりゅう', job: 'tb_fighter', kind: 'support', jp: 300, desc: 'テスト', mods: { twoSwords: true } },
    tb_regen:   { name: 'さいせい', job: 'tb_fighter', kind: 'support', jp: 300, desc: 'テスト', mods: { regen: true } },
    tb_haste:   { name: 'せんせい', job: 'tb_fighter', kind: 'support', jp: 300, desc: 'テスト', mods: { startBuffs: { agi: 1, def: 1 } } },
    tb_saver:   { name: 'MPせつやく', job: 'tb_caster', kind: 'support', jp: 300, desc: 'テスト', mods: { mpCostPct: -50 } },
    tb_booster: { name: 'まほうきょうか', job: 'tb_caster', kind: 'support', jp: 300, desc: 'テスト', mods: { magicPct: 50, healPct: 50 } },
    tb_chemist: { name: 'くすりのちしき', job: 'tb_caster', kind: 'support', jp: 300, desc: 'テスト', mods: { itemPct: 100 } },
    tb_brawn:   { name: 'ばかぢから', job: 'tb_fighter', kind: 'support', jp: 300, desc: 'テスト', mods: { physPct: 50 } },
    ...
    tb_mute:    act('サイレス', 'tb_caster', 'enemy', [{ type: 'status', status: 'silence', chance: 1 }], { mp: 3, magic: true, fx: 'silence' }),
    tb_poison:  act('ポイズン', 'tb_caster', 'enemy', [{ type: 'status', status: 'poison', chance: 1 }], { mp: 2, magic: true, fx: 'poison' }),
    tb_confuse: act('コンフュ', 'tb_caster', 'enemy', [{ type: 'status', status: 'confuse', chance: 1 }], { mp: 3, magic: true, fx: 'confuse' }),
    ...
    tb_dispel:  act('ディスペル', 'tb_caster', 'enemy', [{ type: 'dispel' }], { mp: 4, magic: true, fx: 'dispel' }),
    tb_regenspell: act('リジェネ', 'tb_caster', 'ally', [{ type: 'regen' }], { mp: 5, magic: true, fx: 'regen' }),
    tb_gravity: act('グラビティ', 'tb_caster', 'enemy', [dmg('percent', 0.25)], { mp: 6, magic: true, fx: 'gravity' }),
    tb_doom:    act('デス', 'tb_caster', 'enemy', [{ type: 'status', status: 'death', chance: 1 }], { mp: 8, magic: true, fx: 'death' }),
    tb_bomb:    act('ばくはつ', 'tb_caster', 'enemies', [dmg('fixed', 50, { element: 'fire' })], { mp: 5, fx: 'explosion' }),

    // ---- enemy abilities
    en_tb_fire:   { name: 'ファイア', kind: 'action', target: 'enemy', mp: 2, magic: true, effects: [dmg('magic', 10, { element: 'fire' })], fx: 'fire' },
    en_tb_breath: { name: 'ほのおのいき', kind: 'action', target: 'enemies', effects: [dmg('breath', 20, { element: 'fire' })], fx: 'breath_fire', msg: '{user}は ほのおを はいた！' },
    en_tb_frost:  { name: 'こおりのいき', kind: 'action', target: 'enemies', effects: [dmg('breath', 16, { element: 'ice' })], fx: 'breath_ice', msg: '{user}は つめたい いきを はいた！' },
    en_tb_heal:   { name: 'ヒール', kind: 'action', target: 'ally', mp: 3, magic: true, effects: [{ type: 'heal', power: 40 }], fx: 'heal' },
    en_tb_sleep:  { name: 'ねむりのうた', kind: 'action', target: 'enemies', effects: [{ type: 'status', status: 'sleep', chance: 0.5 }], fx: 'sleep' },
    en_tb_revive: { name: 'よみがえり', kind: 'action', target: 'ally_dead', mp: 5, magic: true, effects: [{ type: 'revive', pct: 1 }], fx: 'revive' },
    en_tb_smash:  { name: 'つうこんのいちげき', kind: 'action', target: 'enemy', effects: [dmg('phys', 2)], fx: 'strike', msg: '{user}は ちからを ためて なぐりかかった！' },
    en_tb_roar:   { name: 'おたけび', kind: 'action', target: 'self', effects: [{ type: 'buff', stat: 'atk', stages: 1 }], fx: 'buff', msg: '{user}は おたけびを あげた！' },
    en_tb_steal:  { name: 'ぬすむ', kind: 'action', target: 'enemy', effects: [{ type: 'steal' }], fx: 'steal' },
  });

  const allOf = (job) => Object.keys(DB.abilities).filter((k) => DB.abilities[k].job === job);
  Object.assign(DB.jobs, {
    tb_fighter: {
      name: 'テストせんし', tier: 1, desc: 'テスト', command: 'わざ',
    ...
    tb_caster: {
      name: 'テストまじゅつし', tier: 1, desc: 'テスト', command: 'じゅもん',
      mult: { hp: 1, mp: 1, str: 1, vit: 1, agi: 1, int: 1, mnd: 1, luk: 1 },
    ...
    tb_adept: {
      name: 'テストたつじん', tier: 2, desc: 'テスト', command: 'ひでん',
      mult: { hp: 1, mp: 1, str: 1, vit: 1, agi: 1, int: 1, mnd: 1, luk: 1 },
  ...
  Object.assign(DB.items, {
    ...
    tb_flame:  { name: 'ほのおのけん', type: 'weapon', wtype: 'sword', atk: 20, element: 'fire', price: 100, desc: 'テスト' },
    tb_venom:  { name: 'どくのナイフ', type: 'weapon', wtype: 'knife', atk: 12, onHit: { status: 'poison', chance: 1 }, price: 100, desc: 'テスト' },
    tb_staff:  { name: 'テストのつえ', type: 'weapon', wtype: 'staff', atk: 5, mag: 5, price: 100, desc: 'テスト' },
    tb_ring:   { name: 'ひよけのわ', type: 'acc', price: 100, desc: 'テスト', mods: { elemResist: { fire: 0.5 }, statusImmune: ['sleep'] } },
    tb_boost:  { name: 'えんのわ', type: 'acc', price: 100, desc: 'テスト', mods: { elemBoost: { fire: 50 } } },
    tb_herb:   { name: 'テストやくそう', type: 'consumable', price: 8, desc: 'テスト', use: { target: 'ally', effects: [{ type: 'heal', power: 30, scale: 0 }], fx: 'heal', battle: true, field: true } },
    ...
    tb_feather: { name: 'テストのはね', type: 'consumable', price: 300, desc: 'テスト', use: { target: 'ally_dead', effects: [{ type: 'revive', pct: 0.5 }], fx: 'revive', battle: true, field: true } },
    tb_firebomb: { name: 'テストばくだん', type: 'consumable', price: 40, desc: 'テスト', use: { target: 'group', effects: [{ type: 'damage', formula: 'fixed', power: 30, element: 'fire' }], fx: 'fire', battle: true, field: false } },
    tb_nut:    { name: 'テストのみ', type: 'consumable', price: 0, rare: true, desc: 'テスト', use: { target: 'ally', effects: [{ type: 'grow', stat: 'agi', n: 2 }], fx: 'grow', battle: true, field: true } },
    tb_gem:    { name: 'きらめくいし', type: 'consumable', price: 0, rare: true, desc: 'テスト', use: { target: 'ally', effects: [{ type: 'healMp', power: 50 }], fx: 'mp', battle: true, field: true } },
  });
  ...
  Object.assign(DB.monsters, {
    ...
    tb_bluejelly: M({ name: 'あおゼリー', sprite: 'jelly', hue: 200, lv: 2, hp: 16, atk: 11, def: 4, agi: 6, exp: 5, gold: 6, jp: 3 }),
    ...
    tb_mage:   M({ name: 'テストまどうし', sprite: 'darkmage', lv: 6, hp: 45, mp: 40, atk: 12, def: 8, mag: 30, agi: 12, exp: 20, gold: 18, jp: 6,
    ...
    tb_thief:  M({ name: 'テストとうぞく', sprite: 'goblin', hue: 120, lv: 4, hp: 30, atk: 14, def: 6, agi: 20, exp: 10, gold: 30, jp: 4, actions: [{ id: 'en_tb_steal', w: 1 }] }),
    ...
    tb_brute:  M({ name: 'かいりき', sprite: 'orc', lv: 30, hp: 3000, atk: 400, def: 100, agi: 200, exp: 0, gold: 0, jp: 0, crit: 0, hit: 999 }),
  });
