...
(function (R) {
  ...
  // ------------------------------------------------------------------ items
  // band 7 = post-game. rare:true → never sold, shown with ★. price 0 = cannot be sold.
  ...
  const PG = { rare: true, price: 0, band: 7 };
  ...
  const items = {
    ...
    pg_chaos_sword:   W('混沌の剣',   'sword',  124, '深淵の底で眠っていた漆黒の剣。\n刃の奥で七色の光が渦を巻く。', { stats: { str: 8, agi: 4 } }),
    pg_dragon_lance:  W('竜神の槍',   'spear',  126, '古の竜神のひげを穂先に鍛えた槍。', { stats: { str: 6, agi: 6 } }),
    pg_void_katana:   W('虚空の太刀', 'katana', 128, '空間ごと断ち切るという妖刀。\n会心の一撃が出やすい。', { stats: { agi: 8 }, mods: { crit: 10 } }),
    pg_ruin_axe:      W('崩界の斧',   'axe',    140, '世界の果てを砕いたと伝わる大斧。', { hit: -5, stats: { str: 10 } }),
    pg_star_dagger:   W('星霜の短剣', 'knife',   90, '幾千の星霜を経て輝く短剣。\n素早さと運が大きく上がる。', { hit: 10, stats: { agi: 16, luk: 10 }, mods: { crit: 6 } }),
    pg_nova_claw:     W('新星の爪',   'claw',   108, '生まれたての星のように燃える爪。', { element: 'fire', stats: { str: 8, agi: 10 } }),
    pg_heaven_bow:    W('天穹の弓',   'bow',    120, '天の弧をかたどった大弓。\n放った矢は決して外れない。', { twoHanded: true, hit: 15, mods: { crit: 6 } }),
    pg_origin_rod:    W('始原のロッド', 'rod',   28, '世界の始まりの光を封じたロッド。\n魔法の威力が大きく上がる。', { mag: 64, stats: { int: 10 }, mods: { magicPct: 15 } }),
    pg_aurora_staff:  W('極光の杖',   'staff',   44, 'オーロラの光を宿す杖。\n回復の力が大きく増す。', { mag: 32, stats: { mnd: 30 }, mods: { healPct: 20 } }),
    pg_genesis_harp:  W('創世の竪琴', 'harp',    72, '世界が生まれた日の調べを奏でる。\n聴いた者を惑わせる。', { stats: { mnd: 16, int: 12 }, onHit: { status: 'confuse', chance: 0.2 } }),

    // armor
    pg_abyss_mail:    A('深淵の鎧',   'heavy',  90, '深淵の闇を鍛えて固めた鎧。\n闇・炎・氷に強い。', { mdef: 20, stats: { vit: 8 }, mods: { elemResist: { dark: 0.5, fire: 0.75, ice: 0.75 } } }),
    pg_starlight_garb: A('星光の衣',  'light',  68, '星の光を織り込んだ軽い衣。\n氷と雷に強い。', { mdef: 18, eva: 8, stats: { agi: 14 }, mods: { elemResist: { ice: 0.5, thunder: 0.5 } } }),
    pg_aurora_robe:   A('極光のローブ', 'robe', 56, 'オーロラの糸で縫ったローブ。\n沈黙を防ぎ、炎・氷・雷を和らげる。', { mdef: 46, stats: { int: 12, mnd: 12 }, mods: { statusImmune: ['silence'], elemResist: { fire: 0.75, ice: 0.75, thunder: 0.75 } } }),
    pg_chaos_helm:    A('混沌の兜',   'helm',   44, '混沌の力を封じた兜。\n眠りと麻痺を防ぐ。', { mdef: 14, mods: { statusImmune: ['sleep', 'paralyze'] } }),
    pg_halo:          A('天輪の冠',   'hat',    32, '天使の輪をかたどった冠。\n眠りと混乱を防ぐ。', { mdef: 26, stats: { int: 10, mnd: 10 }, mods: { statusImmune: ['sleep', 'confuse'] } }),
    pg_void_shield:   A('虚空の盾',   'shield', 48, '虚空を映す盾。\n炎・氷・雷・闇の力を和らげる。', { eva: 12, mdef: 16, mods: { elemResist: { fire: 0.6, ice: 0.6, thunder: 0.6, dark: 0.6 } } }),

    // accessories — the preparation the superboss asks for
    pg_clarity_amulet: X('明鏡の護符', '心を曇りなき鏡のように保つ護符。\n眠り・麻痺・混乱を受けつけない。', { mods: { statusImmune: ['sleep', 'paralyze', 'confuse'] } }),
    pg_soul_bell:     X('魂鎮めの鈴', '魂を体につなぎとめる澄んだ鈴。\n即死・毒・暗闇を防ぎ、最大HPが増える。', { mods: { statusImmune: ['death', 'poison', 'blind'], hpPct: 10 } }),
    ...
    pg_genesis_sword: W('開闢の剣',   'sword',  160, '天地が分かれた時に生まれたという剣。\n深淵の主を討った者だけが手にできる。', { element: 'holy', stats: { str: 15, vit: 10, agi: 10 }, mods: { crit: 8 } }),
    pg_abyss_crest:   X('深淵の紋章', '深淵を越えた証の紋章。\nすべての能力が上がり、あらゆる状態異常を防ぐ。', {
  ...
  Object.assign(R.DB.abilities, {
    ...
    en_pg_howl:        act('冥府の遠吠え', 'enemies', [buff('def', -1, 0.5)], 'debuff', '{user}は冥府に響く遠吠えを上げた！'),
    en_pg_madness_gaze: act('狂気の眼光', 'enemies', [status('confuse', 0.3)], 'confuse', '{user}の目が七色に妖しく光った！'),
    en_pg_prism_beam:  act('虹色の光線', 'enemy', [magic(70, 0.9)], 'holy2', '{user}は虹色の光線を放った！'),
    en_pg_soul_drain:  act('魂吸い', 'enemy', [magic(50, 0.8, 'dark', { drain: 0.5 })], 'drain', '{user}は魂を吸い取ろうとした！'),
    ...
    en_pg_void_breath: act('虚無の息', 'enemies', [breath(120, 'dark')], 'breath_dark', '{user}は虚無の息を吐き出した！'),
    en_pg_frost_breath: act('凍てつく息吹', 'enemies', [breath(120, 'ice')], 'breath_ice', '{user}は凍てつく息吹を吐き出した！'),
    en_pg_flame_breath: act('獄炎の息', 'enemies', [breath(120, 'fire')], 'breath_fire', '{user}は獄炎の息を吐き出した！'),
    ...
    en_pg_lord_flame:  act('創世の業火', 'enemies', [breath(260, 'fire')], 'breath_fire', '{user}は創世の業火を吐き出した！'),
    en_pg_lord_frost:  act('終末の凍気', 'enemies', [breath(260, 'ice')], 'breath_ice', '{user}は終末の凍気を吐き出した！'),
    en_pg_lord_void:   act('虚無の吐息', 'enemies', [breath(300, 'dark')], 'breath_dark', '{user}はすべてを呑み込む虚無の吐息を吐いた！'),
    ...
    en_pg_lord_roar:   act('終焉の咆哮', 'enemies', [magic(160, 0.8)], 'explosion3', '{user}は世界の終わりを告げる咆哮を上げた！'),
  ...
  const X_REG = { exp: 1, gold: 1, jp: 1.5 }; // post-game JP is generous: it is where jobs get mastered

  Object.assign(R.DB.monsters, {
    // ================================================= 地下1階 (Lv45–48)
    pg_hell_hound: pgMon('冥府の番犬', 'wolf', 46, {
      s: { hp: 1.0, atk: 1.35, def: 1.1, agi: 1.5 }, x: X_REG, hue: -30, sat: 1.6, bri: 0.8, eva: 8,
      elem: { fire: 0.5, ice: 1.5, holy: 1.5 }, statusRes: HARD_RES,
      a: [['attack', 4], ['en_pg_hellfire_fang', 3], ['en_pg_howl', 1]],
      drop: ['nectar', 8], rare: ['pg_nova_claw', 96], steal: ['nectar', 'seed_agi'],
      desc: '深淵の入り口を守る冥府の番犬。\n燃える牙で侵入者を焼き噛みにする。',
    }),
    pg_prism_eye: pgMon('虹の魔眼', 'eyeball', 46, {
      s: { hp: 1.2, atk: 1.2, mag: 1.3, mdef: 1.6, agi: 1.2 }, x: X_REG, hue: 150, sat: 1.8, bri: 1.1, mp: 40,
    ...
    pg_void_wraith: pgMon('虚無の亡霊', 'void_wraith', 47, {
      s: { hp: 1.05, atk: 1.2, mag: 1.4, mdef: 1.5, agi: 1.1 }, x: X_REG, mp: 60,
      flags: ['undead', 'flying'], elem: { holy: 2, fire: 1.25, dark: -1, ice: 0.5 }, statusRes: UNDEAD_RES, eva: 10,
    ...
    pg_abyss_knight: pgMon('深淵の騎士', 'armor', 48, {
      s: { hp: 1.2, atk: 1.45, def: 1.7, mdef: 1.1, agi: 0.9 }, x: X_REG, hue: 170, sat: 1.4, bri: 0.75,
    ...
    pg_chaos_beast: pgMon('カオスビースト', 'chaos_beast', 50, {
      s: { hp: 1.0, atk: 1.3, def: 1.1, agi: 1.0 }, x: X_REG,
    ...
    pg_star_golem: pgMon('星鋼の巨兵', 'golem', 50, {
      s: { hp: 1.1, atk: 1.3, def: 1.9, mdef: 1.2, agi: 0.6 }, x: X_REG, hue: 180, sat: 1.5, bri: 1.15,
    ...
    pg_abyss_sorcerer: pgMon('深淵の魔導師', 'darkmage', 50, {
      s: { hp: 0.95, atk: 1.1, mag: 1.5, mdef: 1.6, agi: 1.2 }, x: X_REG, hue: 95, sat: 1.5, bri: 0.9, mp: 90,
    ...
    pg_aurora_harpy: pgMon('極光のハーピー', 'harpy', 49, {
      s: { hp: 1.0, atk: 1.25, mag: 1.2, agi: 1.5 }, x: X_REG, hue: 120, sat: 1.7, bri: 1.1, eva: 10,
    ...
    pg_void_dragon: pgMon('虚竜', 'wyvern', 54, {
      s: { hp: 0.95, atk: 1.3, def: 1.2, agi: 1.1 }, x: X_REG, hue: -150, sat: 0.4, bri: 0.55,
    ...
    pg_chaos_chimera: pgMon('混沌キマイラ', 'chimera', 54, {
      s: { hp: 1.0, atk: 1.35, def: 1.1, agi: 1.2 }, x: X_REG, hue: 160, sat: 1.6, bri: 0.85,
      flags: ['flying'], elem: { ice: 1.5, fire: 0.5 }, statusRes: HARD_RES,
      a: [['attack', 4], ['en_pg_flame_breath', 3], ['en_pg_greed', 2], ['en_pg_howl', 1]],
      drop: ['nectar', 8], rare: ['pg_void_katana', 128], steal: ['nectar', 'seed_str'],
      desc: '獅子と竜と蛇が混沌のうちに結びついた獣。\n三つの口から獄炎を吐く。',
    }),
    pg_doom_box: pgMon('滅びの箱', 'mimic', 55, {
      s: { hp: 1.3, atk: 1.35, def: 1.3, mdef: 1.3, agi: 1.1 }, x: { exp: 1.3, gold: 3, jp: 1.5 }, hue: -60, sat: 1.5, bri: 0.8,
    ...
    pg_arch_demon: pgMon('深淵の魔将', 'demon', 56, {
      s: { hp: 0.9, atk: 1.35, mag: 1.3, mdef: 1.2, agi: 1.1 }, x: X_REG, hue: 100, sat: 1.4, bri: 0.7, mp: 80,
    ...
    pg_abyss_gargoyle: pgMon('深淵のガーゴイル', 'gargoyle', 57, {
      s: { hp: 1.1, atk: 1.3, def: 1.6, mdef: 1.3, agi: 1.2 }, x: X_REG, hue: 130, sat: 1.3, bri: 0.7,
    ...
    rare_prism: pgMon('プリズマ', 'rare_prism', 55, {
      s: { hp: 1.8, atk: 1.0, def: 2.4, mdef: 2.4, agi: 1.6, mag: 1.2 }, x: { exp: 6, gold: 8, jp: 5 }, mp: 40,
      eva: 12, flags: ['rare', 'flee'], fleeRate: 0.4, attackFx: 'strike',
      ...
      drop: ['goddess_tear', 6], rare: ['pg_prism_ring', 48], steal: ['seed_luk', 'pg_prism_ring'],
    ...
    // ================================================= 裏ボス
    abyss_lord: {
      name: '深淵の主アビスロード', sprite: 'boss_abyss', lv: 70, actsPerTurn: 3,
      hp: 16000, mp: 0, atk: 360, def: 180, agi: 110, mag: 300, mdef: 110, eva: 2,
      ...
      actions: [
        ...
        { id: 'en_pg_lord_gather', w: 10000, cond: { every: [9, 2], hpAbove: 0.5 } },
        { id: 'en_pg_lord_gather', w: 10000, cond: { every: [9, 8], hpAbove: 0.5 } },
        ...
        { id: 'en_pg_lord_void', w: 3, cond: { hpBelow: 0.5 } },
      ],
      drop: { item: 'goddess_tear', rate: 1 }, rare: { item: 'light_drop', rate: 1 },
      desc: '世界が生まれる前から深淵に在った\n混沌の竜神。魔王ヴァルザードの力も\nこの者のかけらに過ぎなかったという。',
    },
  });
