...
(function (R) {
  ...
  class BattleScene extends R.Layer {
    ...
    /** 「★レアアイテム！」 popup with the item; stays until A or ↓ (after a short moment, so a held A can't skip it) */
    rarePopup(item) {
      return new Promise((resolve) => { this.popup = { item, t: 0, resolve }; });
    }
    drawPopup() {
      const p = this.popup;
      if (!p) return;
      const it = DB.items[p.item] || { name: p.item };
      const name = (R.Menu && R.Menu.kit ? R.Menu.kit.itemLabel(p.item) : it.name);
      const w = Math.max(150, Math.ceil(G().textWidth(name)) + 56), h = 50;
      const x = (R.W - w) >> 1, y = 58;
      G().window(x, y, w, h, { title: '★レアアイテム！' });
      if (R.Menu && R.Menu.kit) R.Menu.kit.drawIcon(it, x + 14, y + 14);
      G().text(name, x + 26, y + 11, { color: G().C.yellow });
      G().text('を手に入れた！', x + 26, y + 27);
      if (p.t >= 24 && (R.Engine.frame >> 4) & 1) G().text('▼', x + w - 12, y + h - 13, { align: 'right' });
    }
    waitKey() {
      return new Promise((resolve) => {
        const m = this.msg;
    ...
    async handle(ev) {
      ...
      switch (ev.t) {
        ...
        case 'gain':
          ...
          // a rare item always gets a popup that waits for A / ↓, whatever the message speed
          if (ev.rare && ev.item) return s.rarePopup(ev.item);
          return;
        case 'rare':
    ...
    update() {
      if (this.popup) {
        const p = this.popup;
        if (p.t >= 24 && (In().pressed('a') || In().pressed('down'))) {
          R.sfx('confirm_soft');
          if (In().pressed('down')) this.downLatch = true;
          In().consume();
          this.popup = null;
          p.resolve();
        }
