...
(function (R) {
  ...
  function tapestry() {
    return cv(piece(16, 16, (L) => {
      ...
      // cloth: red border band, teal field
      for (let y = 2; y <= 13; y++) for (let x = 2; x <= 13; x++) {
        const edge = x === 2 || x === 13 || y === 2 || y === 13;
        const band = x === 3 || x === 12 || y === 3 || y === 12;
        let c = edge ? RED[2] : band ? GOLD[2] : TEAL[2];
        if (!edge && !band && ((x + y) % 2 === 0) && (x === 4 || x === 11 || y === 4 || y === 11)) c = TEAL[1];
        if (x >= 12) c = tk().mul(c, 0.85);
        L.set(x, y, c);
      }
      for (let x = 3; x <= 12; x += 3) L.set(x, 2, GOLD[3]);
      // tree of life
      grid(L, 4, 4, [
        '...ab...',
        '.abbbba.',
        ...
        '..tttt..',
        '.t.tt.t.',
      ], { a: GOLD[3], b: GOLD[4], r: RED[4], t: GOLD[2] });
      L.set(6, 5, GOLD[5]); L.set(7, 4, GOLD[5]);
      // fringe
  ...
  function windowRect(frame, curtains) {
    ...
    return cv(piece(16, 16, (L) => {
      ...
      if (curtains) {
        const C = RED;
        L.hline(1, 14, 1, WOOD[1]); L.set(1, 1, GOLD[4]); L.set(14, 1, GOLD[3]);
        const side = (x0, dir) => {
          // gathered curtain: wide at the top, tied at y8, flaring below
          const widths = [3, 3, 3, 3, 2, 2, 1, 1, 2, 3, 3];
          widths.forEach((w, i) => {
            const y = 2 + i;
            for (let k = 0; k < w; k++) {
              const x = dir > 0 ? x0 + k : x0 - k;
              L.set(x, y, k === w - 1 ? C[2] : (k + i) % 3 === 0 ? C[4] : C[3]);
            }
          });
          L.set(dir > 0 ? x0 : x0, 8, GOLD[4]); L.set(dir > 0 ? x0 + 1 : x0 - 1, 8, GOLD[3]);
        };
        side(2, 1); side(13, -1);
      }
    }, { drop: [1, 1, 0.3] }));
  ...
  function windowArch(kind) {
    return cv(piece(16, 16, (L, t) => {
      ...
      for (let y = 0; y <= 13; y++) for (let x = 2; x <= 13; x++) {
        ...
        const a = (x + y) & 3, b = (15 - x + y) & 3;
        let c;
        if (kind === 'stained') {
          const cell = ((x + y) >> 2) + ((15 - x + y) >> 2);
          ...
          c = y < 7 ? GLASS[3] : GLASS[2];
          if ((a === 2 && b === 2)) c = GLASS[4];
          if (a === 0 || b === 0) c = IRON[2];
        }
        L.set(x, y, c);
      }
  ...
  function swords() {
    return cv(piece(16, 16, (L) => {
      const sword = (flip) => {
        ...
        for (let i = 0; i < 8; i++) { L.set(X(2 + i), 1 + i, STEEL[5]); L.set(X(3 + i), 1 + i, STEEL[3]); }
        L.set(X(2), 1, STEEL[4]);
        for (const [x, y] of [[13, 7], [12, 8], [11, 9], [10, 10], [9, 11]]) L.set(X(x), y, GOLD[3]);
        L.set(X(13), 7, GOLD[4]);
        L.set(X(12), 11, WOOD[2]); L.set(X(13), 12, WOOD[1]); L.set(X(11), 10, WOOD[2]);
        L.set(X(14), 13, GOLD[4]); L.set(X(13), 13, GOLD[2]);
      };
      sword(false); sword(true);
      // round shield covering the crossing
      for (let y = 4; y <= 12; y++) for (let x = 3; x <= 12; x++) {
        const d = Math.hypot(x - 7.5, y - 8);
        if (d > 4.3) continue;
        L.set(x, y, d > 3.3 ? (x + y < 16 ? GOLD[4] : GOLD[2]) : x + y < 15 ? RED[4] : RED[3]);
      }
      L.rect(7, 7, 2, 2, GOLD[4]); L.set(7, 7, GOLD[5]); L.set(8, 8, GOLD[2]);
    }, { drop: [1, 1, 0.32] }));
  }
  ...
  function straw() {
    return cv(piece(16, 16, (L) => {
      ...
      const pile = (x, y) => ((x - 8) / 7.5) ** 2 + ((y - 9) / 6) ** 2 < 1;
      for (const p of scatter(401, 44, S, pile, (r) => 2 + Math.floor(r() * 3))) {
  ...
  function leaves() {
    return cv(piece(16, 16, (L) => {
      ...
      const pts = [[2, 3], [9, 2], [13, 6], [5, 8], [10, 10], [2, 12], [7, 13], [13, 13]];
      pts.forEach(([x, y], i) => {
  ...
  function stove(f) {
    return piece(16, 16, (L, t, U, O) => {
      ...
      // pot on top
      for (let y = 1; y <= 5; y++) for (let x = 4; x <= 11; x++) {
        const dx = (x - 7.5) / 3.6, dy = (y - 3.4) / 2.4;
        if (dx * dx + dy * dy <= 1) L.set(x, y, x < 7 ? IRON[3] : x < 10 ? IRON[2] : IRON[1]);
      }
      L.hline(5, 10, 1, IRON[4]); L.set(3, 3, IRON[2]); L.set(12, 3, IRON[1]); L.set(6, 2, IRON[5]);
      // steam
  ...
  function sink() {
    return cv(piece(16, 16, (L) => {
      ...
      L.vline(1, 1, 4, CLOTH[3]); L.vline(2, 0, 4, CLOTH[4]); L.set(2, 2, BLUE[4]);
  ...
  function desk() {
    return cv(piece(16, 16, (L) => {
      ...
      // open book
      L.rect(3, 4, 5, 3, CREAM[4]); L.vline(5, 4, 6, CREAM[1]); L.set(3, 6, CREAM[2]); L.set(7, 6, CREAM[2]);
      L.set(4, 5, CREAM[1]); L.set(6, 5, CREAM[1]);
      // ink pot and quill
  ...
  function sacks() {
    return cv(piece(16, 16, (L) => {
      const R_ = [BURLAP[1], BURLAP[2], BURLAP[3], BURLAP[4], BURLAP[5]];
      L.shadeEllipse(10.5, 7.5, 4, 4.3, R_, { dither: 0.5 });
      L.rect(10, 2, 2, 2, BURLAP[2]); L.set(9, 1, BURLAP[4]); L.set(12, 1, BURLAP[3]); L.set(10, 1, BURLAP[5]);
      L.hline(9, 12, 3, WOOD[2]);
      L.shadeEllipse(6, 10.5, 4.6, 4, R_, { dither: 0.5 });
      L.rect(5, 5, 2, 2, BURLAP[2]); L.set(4, 4, BURLAP[4]); L.set(7, 4, BURLAP[3]); L.set(5, 4, BURLAP[5]);
      L.hline(4, 7, 6, WOOD[2]);
      L.shadeEllipse(12.5, 12.5, 2.8, 2.2, R_, { dither: 0.5 });
      // stencil mark, seam
      L.set(5, 10, BURLAP[1]); L.set(7, 10, BURLAP[1]); L.set(6, 11, BURLAP[1]); L.set(5, 12, BURLAP[1]); L.set(7, 12, BURLAP[1]);
      L.vline(12, 6, 10, BURLAP[2]);
      // spilled grain
      for (const [x, y] of [[1, 14], [2, 13], [3, 14], [2, 15], [4, 15]]) L.set(x, y, (x + y) % 2 ? GOLD[4] : GOLD[3]);
    }, { contact: [8.5, 14.2, 7.6, 1.8, 0.36] }));
  }
  ...
  function armorStand() {
    return cv(piece(16, 32, (L) => {
      ...
      grid(L, 2, 1, [
        ...
        '..wwwwwwwwww',
        '..vvvvvvvvvv',
      ], { r: RED[4], q: RED[2], a: S[5], b: S[4], c: S[3], d: S[2], e: S[5], k: IRON[0], g: GOLD[3], w: WOOD[4], v: WOOD[2] });
  ...
  function weaponRack() {
    return cv(piece(16, 32, (L) => {
      // spear
      L.vline(4, 4, 28, WOOD[4]); L.set(4, 1, STEEL[5]); L.vline(4, 2, 3, STEEL[4]); L.set(3, 3, STEEL[3]); L.set(5, 3, STEEL[2]);
      L.set(4, 4, RED[3]); L.set(4, 5, RED[2]);
      // sword
      L.set(7, 6, STEEL[5]); L.vline(7, 7, 20, STEEL[5]); L.vline(8, 7, 20, STEEL[3]); L.set(8, 6, STEEL[4]);
      L.hline(5, 10, 21, GOLD[3]); L.set(5, 21, GOLD[4]);
      L.vline(7, 22, 25, WOOD[1]); L.vline(8, 22, 25, WOOD[0]); L.set(7, 26, GOLD[4]); L.set(8, 26, GOLD[2]);
      // axe
      L.vline(11, 8, 28, WOOD[3]);
      grid(L, 11, 7, ['.ab', 'abb', 'abc', 'abc', '.bc', '..c'], { a: STEEL[5], b: STEEL[4], c: STEEL[2] });
      L.set(10, 9, STEEL[3]);
      // rack frame
      L.vline(1, 12, 29, WOOD[4]); L.vline(2, 12, 29, WOOD[2]);
      L.vline(13, 12, 29, WOOD[3]); L.vline(14, 12, 29, WOOD[1]);
      L.hline(1, 14, 12, WOOD[5]); L.hline(1, 14, 13, WOOD[2]);
      for (const x of [4, 7, 8, 11]) L.set(x, 13, WOOD[0]);
      L.hline(1, 14, 26, WOOD[4]); L.hline(1, 14, 27, WOOD[1]);
      L.hline(0, 15, 29, WOOD[3]); L.hline(0, 15, 30, WOOD[1]);
  ...
  function candelabra(f) {
    return piece(16, 32, (L, t, U) => {
      const fx = [3.5, 7.5, 11.5], fy = [4, 3, 4];
      for (let i = 0; i < 3; i++) glow(U, fx[i], fy[i], f ? 3.6 : 4.2, 0.18);
      // candles
      for (const [x, y0] of [[3, 6], [7, 5], [11, 6]]) {
        L.rect(x, y0, 2, 5 - (y0 === 5 ? 0 : 1) + 1, CREAM[4]); L.vline(x + 1, y0, y0 + 4, CREAM[2]);
        L.set(x, y0, CREAM[3]);
        const h = f ? 3 : 4;
        for (let k = 1; k <= h; k++) L.set(x + (k === h && f ? 1 : 0), y0 - k, k === 1 ? FIRE[4] : k === h ? FIRE[2] : FIRE[3]);
        L.set(x, y0 - 1, FIRE[5]);
      }
      // cups and arms
      for (const x of [2, 6, 10]) { L.hline(x, x + 3, 11, GOLD[4]); L.hline(x + 1, x + 2, 12, GOLD[2]); }
      L.hline(3, 12, 14, GOLD[3]); L.set(3, 13, GOLD[3]); L.set(12, 13, GOLD[2]); L.hline(4, 11, 15, GOLD[2]);
      // stem with knops
      L.vline(7, 13, 27, GOLD[4]); L.vline(8, 13, 27, GOLD[2]);
      for (const y of [17, 23]) { L.hline(6, 9, y, GOLD[3]); L.set(6, y, GOLD[5]); L.set(9, y, GOLD[1]); }
      // tripod foot
  ...
  function daisCell(e, P, u0) {
    ...
    for (let j = 0; j < top; j++) for (let i = 0; i < 16; i++) {
      img.put(i, j, 0xffffff, 0.06);
      if (!e.N && j === 0) img.put(i, j, 0xffffff, 0.34);
      if (!e.N && j === 1) img.put(i, j, 0x000000, 0.12);
      if (!e.W && i === 0) img.put(i, j, 0xffffff, 0.26);
      if (!e.E && i === 15) img.put(i, j, 0x000000, 0.34);
    }
    if (riser) {
      const rows = [P[4], P[2], P[1], P[3], P[2], P[1]];
      for (let j = 0; j < 6; j++) for (let i = 0; i < 16; i++) {
        let c = rows[j];
        const u = u0 + i;
        if ((j === 1 || j === 2 || j === 4 || j === 5) && (u & 7) === 7 && P !== null) c = P[0];
        if (!e.W && i === 0) c = j === 0 || j === 3 ? P[4] : P[3];
        if (!e.E && i === 15) c = P[0];
        img.put(i, 10 + j, c, 1);
      }
    }
  ...
  function benchCell(l, r) {
    return piece(16, 16, (L, t, U) => {
      ...
      for (let q = x0; q <= x1; q++) { U.put(q, 12, SH, 0.3); U.put(q, 13, SH, 0.26); }
