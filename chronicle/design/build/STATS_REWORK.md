...
> **リードの決定（2026-09-26）** — §11 の問いへの回答:
> 1. 回復は精神のまま（攻撃の術＝知力 4.5%/点、回復＝精神 4%/点）。
> 2. 25 を持つ 5 人（ハーゲン腕力・ドッカ体力・ティッタ器用さ・テオ知力・マルタ精神）は案のとおり。
> 3. デメリットを残す 36 品は案のとおり。
> 4. 本編クリアで主な武器 ≈70（P2 下限 68）でよい。
> 5. **ボスの盗み専用品も 1/16〜1/32（オーナー A31「甘くしない」）。** 1/8 は使わない。戦闘中は何度でも盗みを試せる（1 回きりにはしない）。§7 の表の boss 行はこれに読み替える。
> 6. 標準の模擬パーティはバルトロ（大剣）で入れ替え。
> 7. HP/MP の伸びは「こまめに少しずつ」（1 戦で +1〜数点、強敵・ボスで確実に）。
> 8. 出どころを失う品は約 30 ずつ宝箱・ボスへ移し、残りは削除でよい。
