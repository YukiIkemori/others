...
for (const k of Object.keys(NORMAL_NAMES)) NORMAL_NAMES[k] = NORMAL_NAMES[k].split(' ');
...
// §8.1.3・§9.12.6 の数
const COUNTS = { normal: 120, rare: 59, super: 10, mrare: 31, msuper: 79, fixed: 2, total: 301 };
...
const GOOD_UP = ['atk', 'def', 'mdef', 'hit', 'eva', 'crit', 'spd', 'mag', 'strPct', 'vitPct', 'dexPct', 'agiPct', 'intPct', 'mndPct', 'hpPct', 'mpPct', 'wpPct',
  'defPct', 'mdefPct', 'physPct', 'magicPct', 'healPct', 'itemPct', 'expPct', 'goldPct', 'dropPct', 'rarePct', 'superPct', 'rareEncPct', 'goldenPct',
  'preemptPct', 'escapePct', 'stealPct', 'autoSteal', 'mpRegen', 'wpRegen', 'autoRevive', 'autoCounter', 'walkHeal', 'drain'];
const GOOD_DOWN = ['mpCostPct', 'wpCostPct', 'takenPct', 'hpLoss'];
/**
...
function classify(it) {
  ...
  for (const k of WEAPON_FIELDS) if (it[k] !== undefined) one(k, it[k]);
...
const CAP = {
  rare: {
    ...
    statusImmune: (v) => v.length === 1, hpPct: (v) => v <= 15, mpPct: (v) => v <= 20, wpPct: (v) => v <= 20, regen: (v) => v === true,
    mpRegen: (v) => v <= 3, wpRegen: (v) => v <= 3, startBuffs: (v) => Object.keys(v).length === 1 && Object.values(v).every((x) => x === 1),
    physPct: (v) => v <= 15, magicPct: (v) => v <= 15, healPct: (v) => v <= 35, itemPct: (v) => v <= 50, mpCostPct: (v) => v >= -25, wpCostPct: (v) => v >= -25,
  ...
  super: {
    ...
    statusImmune: (v) => v.length <= 3, hpPct: (v) => v <= 20, mpPct: (v) => v <= 25, wpPct: (v) => v <= 25, regen: (v) => v === true,
    mpRegen: (v) => v <= 3, wpRegen: (v) => v <= 3, startBuffs: (v) => Object.keys(v).length <= 2 && Object.values(v).every((x) => x === 1),
    physPct: (v, it) => v <= (it._id === 'w_greatsword_sr_frenzy' ? 50 : 25), magicPct: (v) => v <= 25, healPct: (v) => v <= 50, itemPct: (v) => v <= 50,
    mpCostPct: (v) => v >= -35, wpCostPct: (v) => v >= -35, mag: (v, it) => v <= 2 * T_(it),
...
const QCAP = {
  rare: {
    ...
    expPct: (v) => v >= -25, goldPct: (v) => v >= -25, encounterPct: (v) => v <= 50, mpCostPct: (v) => v <= 25, wpCostPct: (v) => v <= 25,
  ...
  super: {
    ...
    mpCostPct: (v) => v <= 50, wpCostPct: (v) => v <= 50, glimPct: (v) => Object.values(v).every((x) => x >= -100), twoHanded: (v) => v === true,
...
const SETS_T8 = {
  ...
  dex: { stat: 'dex', w1: 'w_dagger_sr_moonfang', w2: 'w_whip_sr_silk', shield: 'sh_sr_phantom', head: 'hd_sr_heaveneye', body: 'bd_sr_shadow', hands: 'hn_sr_hundred', feet: 'ft_sr_whirl', acc: ['ac_sr_eagle', 'ac_sr_needle'] },
};
...
module.exports = {
  ROOT, MY_FILES, W, U, PRICE, GRADE_MULT, PRICE_MULT, gearStat, WTYPES, TWO_HANDED, WEAPON_STATS, SERIES_UNITS,
