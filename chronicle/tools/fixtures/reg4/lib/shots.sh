...
timeout 120 node tools/shot.js --html "$HTML" --wait 800 \
  --eval "(async()=>{R=RPG; await R.debug.quickStart({map:'loch', spawn:'inn', noEncounter:true}); R.Settings.fieldZoom='$Z'; $EXTRA; await R.debug.warp('$MAP', $SPJ); return R.debug.pos();})()" \
