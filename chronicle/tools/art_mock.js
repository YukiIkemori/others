...
//
//   node tools/art_mock.js [--page proto2.html] [--styles anime,chibi,storybook] [--scenes battle,town,world,dungeon] [--anim [--clips JSON] [--prefix p]]
...
async function main() {
  ...
  const page0 = arg('--page', 'proto.html');
  const base = `http://127.0.0.1:${srv.address().port}/${page0}`;
  ...
  if (argv.includes('--anim')) {
    ...
    const CLIPS = JSON.parse(arg('--clips', '{"idle":1.6,"attack":1.5,"hurt":1.0}'));
    const pre = arg('--prefix', 'anim_');
    ...
    for (const [clip, len] of Object.entries(CLIPS)) {
      ...
      spawnSync(ff, ['-y', '-framerate', '30', '-i', path.join(adir, `${clip}_%03d.png`), '-vf', 'scale=512:-1:flags=lanczos', '-loop', '0', '-quality', '85', path.join(DIR, 'mocks', `${pre}${clip}.webp`)]);
      spawnSync(ff, ['-y', '-framerate', '30', '-i', path.join(adir, `${clip}_%03d.png`), '-vf', 'scale=512:-1:flags=lanczos,split[a][b];[a]palettegen=max_colors=192[p];[b][p]paletteuse=dither=sierra2_4a', '-loop', '0', path.join(DIR, 'mocks', `${pre}${clip}.gif`)]);
    ...
    for (const style of arg('--styles', 'anime,chibi,storybook').split(',')) {
      for (const scene of arg('--scenes', 'battle,town,world,dungeon').split(',')) {
        const out = path.join(DIR, 'mocks', `${style}_${scene}${arg('--suffix', '')}.png`);
