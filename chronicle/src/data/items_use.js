// 道具（type:'consumable'）56 = §8.9 の 33 ＋ §8.9.2 のレア魔物の道具 23（A18 で気力の茶・気力の実を削除）。担当 gear-b（A10b）。
...
// - grade/src: 店の 28 品 = normal/shop、命のしずく・天の恵み・よみがえりの花・実 2 = rare/drop（店に置かない。宝箱・ボス・魔物）、
...
// - sort は道具の一覧の並び（回復 → 生き返り → MP → 治療 → 香 → 戦闘の道具 → 魔石 → 実 → レア魔物の道具）。
(function (R) {
  'use strict';
  ...
  const USE = {
    ...
    i_lifedew:  C(DROP, '命のしずく', 600, BF('ally', [heal(1), healMp(1)], 'heal'), '味方1人のHPとMPを\nすべて回復する。', { icon: 'drop' }),
  ...
  // 最大値を伸ばす実（フィールドだけ。bonus.hp ≤ 200・mp ≤ 50。§8.2.5）
  Object.assign(USE, {
    i_seed_hp: C(DROP, '活力の実', 0, F('ally', [grow('hp', 10)], 'heal'), '食べると最大HPが\n10増える。', { icon: 'seed' }),
    i_seed_mp: C(DROP, '魔力の実', 0, F('ally', [grow('mp', 3)], 'mp'), '食べると最大MPが\n3増える。', { icon: 'seed' }),
  });
  ...
  Object.assign(USE, {
    i_jewel_carrot:   RM('rm_jewel_hare', '宝石にんじん', 1000, BF('ally', [heal(1), healMp(1)], 'heal'), '味方1人のHPとMPを\nすべて回復する。', 'seed'),
    ...
    i_moon_wool:      RM('rm_moon_sheep', '月見の毛糸', 1500, BF('ally', [healMp(0.5)], 'mp'), '味方1人のMPを\n最大値の50%回復する。', 'rope'),
    i_spring_key:     RM('rm_clock_bird', 'ぜんまいの鍵', 1500, BF('allies', [healMp(0.2)], 'mp'), '味方全員のMPを\n最大値の20%回復する。', 'key'),
    i_wisdom_page:    RM('rm_bookworm', '知恵のページ', 3000, BF('allies', [healMp(0.5)], 'mp'), '味方全員のMPを\n最大値の50%回復する。', 'book'),
    i_golden_ink:     RM('rm_golden_quill', '黄金のインク', 4000, BF('allies', [healMp(0.7)], 'mp'), '味方全員のMPを\n最大値の70%回復する。', 'potion'),
    i_memory_bubble:  RM('rm_memory_fish', '思い出のしゃぼん', 6000, BF('allies', [heal(1), healMp(1)], 'heal3'), '味方全員のHPとMPを\nすべて回復する。', 'drop'),
    i_dream_fruit:    RM('rm_dream_tapir', '夢の果実', 0, F('ally', [grow('hp', 20), grow('mp', 8)], 'heal'), '食べると最大HPが20、\n最大MPが8増える。', 'seed'),
  });
