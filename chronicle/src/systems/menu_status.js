  ...
  function build() {
    ...
    class StatusScreen extends K.Screen {
      ...
      page4(c) {
        ...
        // mastered jobs' signature abilities this member carries into every job (always on)
        const tr = R.Rules.signatures(c).map((id) => DB.abilities[id].name);
        if (tr.length) K.fitText('常時：' + tr.join('・'), 14, 205, 150, { color: G().C.gold });
      }
      page1(c, st) {
    ...
    class BookScreen extends K.Screen {
      ...
      detail1(m, r, b) {
        ...
        // 「ドロップ 薬草／★四つ葉のお守り」 and 「盗める 薬草／★…」: everything this monster can give,
        // each item by name once obtained (by drop or by stealing), 「？？？」 until then
        const rareOk = gotSlot(m, r, 'rare'), dropOk = gotSlot(m, r, 'drop');
        const dparts = [];
        if (m.drop && m.drop.item) dparts.push(dropOk ? K.itemLabel(m.drop.item) : '？？？');
        if (m.rare && m.rare.item) dparts.push(rareOk ? '★' + K.itemLabel(m.rare.item).replace(/★$/, '') : '★？？？');
        G().text('ドロップ', 14, 152, { color: G().C.gray });
        K.fitText(dparts.join('／') || (b ? 'なし' : '？？？'), 82, 152, 160, { color: dropOk || rareOk ? G().C.white : G().C.dark });
        const [sk, sv, scol] = m.steal && (m.steal.item || m.steal.rare) ? stealLine(m, r) : ['盗める', b ? 'なし' : '？？？', G().C.dark];
        G().text(sk, 14, 165, { color: G().C.gray });
        K.fitText(sv, 82, 165, 160, { color: scol });
